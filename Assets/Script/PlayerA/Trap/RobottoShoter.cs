    ...
    void ShootForward()
    {
        ...
        if (Physics.Raycast(origin, dir, out RaycastHit hit, Mathf.Infinity, hitMask, QueryTriggerInteraction.Collide))
        {
            ...
            if (root.CompareTag(playerTag))
            {
                ...
                if (hp != null)
                {
                    if (debugLogs) Debug.Log("[Damage] TakeDamage 呼びます");
                    hp.TakeDamage(damage);
                }
                else
                {
                    if (debugLogs) Debug.LogWarning("[Damage] Player_HP が見つからない");
                }
            }
            else
            {
                if (debugLogs) Debug.Log("[Hit] Playerじゃない物に当たってる");
            }
        }
        else
        {
            if (debugLogs) Debug.Log("[Hit] 何にも当たってない");
        }
    }
